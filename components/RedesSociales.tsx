...
export default function RedesSociales({ variant = "light" }: { variant?: "light" | "dark" }) {
  const color = variant === "dark" ? "text-[var(--color-bordo)]" : "text-[var(--color-papel)]";
  const bg = variant === "dark" ? "bg-[var(--color-bordo)]/10 border-[var(--color-bordo)]/30 hover:bg-[var(--color-bordo)]/20" : "bg-[var(--color-hueso)]/15 border-[var(--color-hueso)]/40 hover:bg-[var(--color-hueso)]/25";
  return (
    <div className="flex items-center justify-center gap-3">
      {redes.map((red) => (
        <Link
          ...
          className={`w-9 h-9 rounded-full border flex items-center justify-center transition ${color} ${bg}`}
        >
          {red.icon}
        </Link>
      ))}
